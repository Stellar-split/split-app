...
export default function RecipientPayoutTracker({ invoice, publicKey, network = "testnet" }: Props) {
  ...
  const getStatus = (recipient: Recipient): PayoutStatus => {
    ...
    if (invoice.status === "Released") {
      const claimedAddresses: string[] =
        (invoice as unknown as { claimed?: string[] }).claimed ?? [];
  ...
  const statusStyles: Record<PayoutStatus, { badge: string }> = {
    Pending: { badge: "bg-yellow-500/20 text-yellow-300" },
    Paid: { badge: "bg-green-500/20 text-green-300" },
    Claimed: { badge: "bg-indigo-500/20 text-indigo-300" },
    Refunded: { badge: "bg-gray-500/20 text-gray-300" },
  };
  ...
  const handleClaim = async (invoiceId: string) => {
    ...
    try {
      ...
      const result = await (
        splitClient as unknown as {
          claimShare: (id: string) => Promise<{ txHash?: string }>;
        }
      ).claimShare(invoiceId);
      ...
      recipients.map((r) => {
        ...
        return (
          <RecipientDetailRow
            ...
            status={status}
            statusBadgeClass={styles.badge}
            isCurrentWallet={isWallet}
        ...
        <table className="w-full text-sm border border-gray-700 rounded-xl overflow-hidden">
          <thead>
            <tr className="bg-gray-800/60 border-b border-gray-700 text-gray-400 text-xs uppercase tracking-wide">
              ...
              <th scope="col" className="px-2 py-3 w-8" aria-label="Expand" />
